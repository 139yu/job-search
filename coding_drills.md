...
## 建议排期（2026-08-19 重排，与 skill_improvement_plan.md 对齐）

> VisionBench 为整合壳：骨架先行，2D halcon 功能作为壳内模块逐个填充，用离线样例图验证。完成阈值：壳 + 3~4 个高质量模块。3D 复跑与点云测量服务已取消。

VisionBench 主线（壳 + 2D halcon 进壳）：

- 第 3 周（8/17-8/23）：图像源架构（进行中）→ FileImageProvider 打通打开/保存/显示
- 第 4 周（8/24-8/30）：第 2 项 Region 导航 + 状态锁（壳验收）→ 画 ROI + 几何测量（两点距离/三点圆直径）
- 第 5 周（8/31-9/6）：卡尺测量（第 11 项核心，measure_pos 测宽/径）+ 像素比例转 mm
- 第 6 周（9/7-9/13）：2D 扩展（阈值分割 + Blob 或 模板匹配定位，二选一）
- 第 7 周（9/14-9/20）：2D 测量闭环最终验收 + 整体走查
- 有余力再做：第 1 项 FPD 状态机、第 3 项 RBAC、第 4 项 Catalog、第 5/7/8/9 项、自绘非模态 ROI
- 已取消：3D 复跑（12-14 项）、点云测量服务（15 项）、鲁棒拟合对比（16 项）
